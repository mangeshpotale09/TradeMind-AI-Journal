...
import { getRegisteredUsers, getStoredTrades, updateUserStatus, exportUsersToCSV, getTransactions } from '../services/storageService';
...
const AdminView: React.FC<AdminViewProps> = ({ onLogout }) => {
  ...
  const loadAdminData = async () => {
    setIsLoading(true);
    const [fetchedUsers, fetchedTrades, fetchedTxs] = await Promise.all([
      getRegisteredUsers(),
      getStoredTrades(),
      getTransactions()
    ]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalRevenue,
      conversionRate: users.length > 0 ? ((paidUsers / users.length) * 100).toFixed(1) : '0'
    };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      {isLoading && <div className="text-center py-10 text-slate-500 font-black uppercase text-[10px] animate-pulse">Syncing Admin Console...</div>}

      {activeSubTab === 'overview' && !isLoading && (
      ...
      {activeSubTab === 'registry' && !isLoading && (
        <div className="bg-[#0e1421] border border-[#1e293b] rounded-3xl overflow-hidden">
